...
<body>
    <form action="{{ route('complaints.store') }}" method="POST" class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <input type="text" value="{{ $request->complainterName }}" name="complainterName" class="form-control"
                disabled id="inputEmail4">
        </div>
        <div class="mb-3">
            <label for="exampleFormControlTextarea1" class="form-label">Response</label>
            <textarea name="Response" class="form-control" id="exampleFormControlTextarea1" rows="3"></textarea>
        </div>
        <div class="mb-3">
            <label for="" class="form-label">Email</label>
            <input type="email" class="form-control" name="email" id="" aria-describedby="emailHelpId"
                disabled value="{{ $request->email }}">
        </div>
